...
class clock(object):
	...
	Implements a clock.
	"""
	def __init__(self, countUp=False, maxSeconds=86399.999, resolution=0.01, hoursFlag=False, clockName='generic', internalClock=False):
		...
		self.timeListNames=['daysTens', 'daysUnits', 'hoursTens', \
	...
	def Kill(self):
		'''Kills update thread'''
		if (_platform == "linux" or _platform == "linux2") and (self.clockName=='periodClock' or self.clockName=='shotClock'):
			os.nice(1)
		self.refresher.resume()
		self.refresher.kill()
...
class clockThread(Thread):
	...
	def run(self):
		...
		while not self._stopevent.isSet():
			...
			stamp+=1
			#startTime=time.time()
			#print name, 'stamp1', stamp, self.nextCall-1486587172
			self.nextCall=self.nextCall+self.period
			#print name, 'stamp2', stamp, self.nextCall-1486587172
			self.function()
			endTime=time.time()
			...
			try:
				...
				#print name, 'threadTimer sleep error is', err, 'for', self.function
				#print name, 'stamp3', stamp, self.nextCall-1486587172, 'nextCall-time.time()', self.nextCall-now
				while (self.nextCall-now)<0:
					self.nextCall=self.nextCall+self.period
					count+=1
				#print name, 'stamp4', stamp, 'count', count
				#print name, 'stamp5', stamp, self.nextCall-1486587172
				self.nextCall=self.nextCall+self.period*count
				#print name, 'stamp6', stamp, self.nextCall-1486587172
				#self.nextCall=self.nextCall+period*(count+1)
				#print name, 'stamp7', stamp, 'count', count, self.nextCall-1486587172, self.nextCall-now
				try:
					now=time.time()
					time.sleep(self.nextCall-now)
				except:
...
def test():
	...
	while periodClock.running:
		...
		print periodClock.timeUnitsDict['minutesTens'], periodClock.timeUnitsDict['minutesUnits'], periodClock.timeUnitsDict['secondsTens'], \
		periodClock.timeUnitsDict['secondsUnits'], periodClock.timeUnitsDict['tenthsUnits'], periodClock.timeUnitsDict['hundredthsUnits']
		time.sleep(0.01)
